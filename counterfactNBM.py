...
accuracy = accuracy_score(fair_predictions,y_test)
...
dice_model = dice_ml.Model(model=alch2,backend='sklearn')
...
y_pred_original = alch2.predict(subset_original)
y_pred_flipped = alch2.predict(subset_flipped)
...
plt.title('Prediction Change per Instance for bias-free model (Race Set to African-American(0) and Sex set to Male(1))')
